...
<div class="user-form create_box">
    ...
    <?= $form->field($model, 'icon',['template' => '{label} <div class="row"><div class="col-sm-12">{input}<button type="button" class="layui-btn upload_button" id="test3"><i class="layui-icon"></i>上传图片</button>{error}{hint}</div></div>'])->textInput(['maxlength' => true,'class'=>'layui-input upload_input']) ?>
    <div class="form-group">
        <?= Html::img(@$model->icon, ['width'=>'50','height'=>'50','class'=>'site_icon'])?>
    </div>
