...
class ActivitySearch extends Activity
{
    ...
    public function search($params)
    {
        ...
        $query = Activity::find()->innerJoinWith(['product'=>function($query){
            $query->andFilterWhere(['like', 'name', $this->productName])->andFilterWhere(['like', 'sku', $this->productSku]);
        }])->where(['type'=>Activity::CASHBACK_COUPON_ACTIVITY,'t_activity.site_id'=>\Yii::$app->session['default_site_id']]);
        if ($role_id == 3)
        {
